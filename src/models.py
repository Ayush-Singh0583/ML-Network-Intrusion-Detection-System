...
def make_norm(kind: str):
    ...
    ``"ln"`` (LayerNorm) is the default for the classifiers, and the reason is
    open-set detection rather than optimisation.

    BatchNorm at inference normalises with running statistics estimated from
    training batches.  On CIC-IDS2017 those batches are ~80% BENIGN, so the
    running mean and variance describe benign traffic.  An out-of-distribution
    flow is then rescaled *toward* that benign reference before it ever reaches
    the classifier head, which destroys exactly the distance-from-manifold
    signal that a logit-based novelty score depends on.

    Measured on a hard 12-class ablation at CIC-IDS2017-like imbalance
    (majority 86.7%, rarest class n=11), near-OOD drawn inside a known class's
    cloud (the DDoS-vs-DoS-Hulk case):

        scorer              BN near   BN far   LN near   LN far
        msp                   0.112    0.547     0.578    0.894
        energy                0.120    0.739     0.630    0.989
        mahalanobis_embed     0.914    1.000     0.930    0.991

    Both logit-based scorers are INVERTED (AUROC < 0.5) under BatchNorm and
    recover under LayerNorm.  Classification cost is small: macro-F1 0.870 ->
    0.857 on the same ablation.  Set ``--norm bn`` to restore the old behaviour.
    """
    kind = kind.lower()
    if kind in ("ln", "layer", "layernorm"):
        return lambda d: nn.LayerNorm(d)
    if kind in ("bn", "batch", "batchnorm"):
        return lambda d: nn.BatchNorm1d(d)
    if kind in ("none", "identity"):
        return lambda d: nn.Identity()
    raise ValueError(f"unknown norm {kind!r}; expected ln | bn | none")
...
class ResidualBlock(nn.Module):
    """Pre-activation residual block: x + W2(drop(act(N(W1(act(N(x))))))) ."""

    def __init__(self, dim: int, dropout: float = 0.2, norm: str = "ln"):
        super().__init__()
        N = make_norm(norm)
        self.bn1 = N(dim)
...
class MLP(nn.Module):
    def __init__(
        ...
        self.blocks = nn.Sequential(
            *[ResidualBlock(hidden, dropout, norm) for _ in range(depth)]
        )
...
class CNN1D(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.n_classes = n_classes
        # Conv feature maps are (B, C, F); GroupNorm(1, C) == LayerNorm over
        # (C, F) and, like LayerNorm, is per-sample -- so it carries the same
        # OOD-preserving property as make_norm("ln") in the dense stacks.
        def CN(c):
            k = norm.lower()
...
def build_model(name: str, input_dim: int, n_classes: int, cfg) -> nn.Module:
    name = name.lower()
    norm = getattr(cfg, "norm", "ln")
    if name == "mlp":
        return MLP(input_dim, n_classes, hidden=cfg.hidden, depth=cfg.depth,
                   dropout=cfg.dropout, norm=norm)
    if name == "cnn":
        return CNN1D(input_dim, n_classes, dropout=cfg.dropout, norm=norm)
    if name == "lstm":
        return LSTMNet(input_dim, n_classes, dropout=cfg.dropout, norm=norm)
    if name == "autoencoder":
