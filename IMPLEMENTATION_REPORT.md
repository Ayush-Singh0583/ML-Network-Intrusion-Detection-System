...
| :--- | :--- | :--- |
| [`src/preprocessing.py`](file:///a:/ML-Network-Intrusion-Detection-System/src/preprocessing.py) | `remove_identifier_columns` | Added `"Destination Port"` to dropped identifiers to prevent shortcut learning. |
| [`src/training.py`](file:///a:/ML-Network-Intrusion-Detection-System/src/training.py) | `train_xgboost`, `train_lightgbm` | Added balanced sample weights for XGBoost and dynamic `num_class` for LightGBM. |
| [`src/evaluation.py`](file:///a:/ML-Network-Intrusion-Detection-System/src/evaluation.py) | `evaluate_model`, `feature_importance` | Added Macro metrics, Unknown Attack detection via confidence thresholding, robust label decoding, and flexible argument defaults. |
| [`src/main.py`](file:///a:/ML-Network-Intrusion-Detection-System/src/main.py) | Top-level execution pipeline | Updated calls to `evaluate_model`, `feature_importance`, and `save_model` to supply all required arguments. |
| [`src/main_rf.py`](file:///a:/ML-Network-Intrusion-Detection-System/src/main_rf.py) | Top-level execution pipeline | Updated `evaluate_model` to pass `encoder` and `model_name`. |
| [`src/main_xgb.py`](file:///a:/ML-Network-Intrusion-Detection-System/src/main_xgb.py) | Top-level execution pipeline | Updated `evaluate_model` to pass `encoder`, `unknown_threshold=0.55`, and `model_name`. |
| [`src/main_lgbm.py`](file:///a:/ML-Network-Intrusion-Detection-System/src/main_lgbm.py) | Top-level execution pipeline | Fixed `evaluate_model` and `feature_importance` argument signatures. |
| [`src/compare_models.py`](file:///a:/ML-Network-Intrusion-Detection-System/src/compare_models.py) | Top-level model benchmark loop | Added Macro Precision, Macro Recall, and Macro F1 to comparison table export. |
| [`src/evalAlone.py`](file:///a:/ML-Network-Intrusion-Detection-System/src/evalAlone.py) | `evaluate_alone` | Supplied `encoder` to `evaluate_model` call. |
| [`backend/live/statistics.py`](file:///a:/ML-Network-Intrusion-Detection-System/backend/live/statistics.py) | `calculate_iat` | Multiplied inter-arrival timestamp deltas by $10^6$ to output microseconds. |
| [`backend/live/flow_manager.py`](file:///a:/ML-Network-Intrusion-Detection-System/backend/live/flow_manager.py) | `process_packet` | Converted active and idle interval gaps to microseconds before appending to flow buffers. |
| [`backend/live/extractor.py`](file:///a:/ML-Network-Intrusion-Detection-System/backend/live/extractor.py) | `extract_features` | Assigned `Flow Duration` in microseconds ($10^6\times\text{seconds}$). |
| [`backend/services/model_service.py`](file:///a:/ML-Network-Intrusion-Detection-System/backend/services/model_service.py) | `predict_dataframe` | Added `"Source Port"` and `"Destination Port"` to dropped identifier columns during CSV prediction. |
...
- [x] **Project builds & runs:** All modules import without syntax or type errors.
- [x] **Random split executes:** `split_dataset` and multi-class training function correctly.
- [x] **Cross-day executes:** `split_dataset_by_day` and evaluation handle string test labels and known/unknown label unions seamlessly.
- [x] **Destination Port removed:** Verified absent from extracted features and training matrices.
- [x] **Model serialization works:** Models, scalers, encoders, and updated feature names serialize/deserialize with `joblib`.
- [x] **Confusion matrix saved:** Heatmaps exported to `results/confusion_matrix.png`.
- [x] **Classification report saved:** Per-class precision, recall, f1, and macro/weighted metrics exported to `results/classification_report.csv`.
- [x] **Unknown detection verified:** Low-confidence predictions successfully mapped to `Unknown_Attack` and logged.
- [x] **Live extractor timing verified:** Microsecond scales confirmed for Flow Duration, IAT, Active, and Idle statistics.
